...
Pty0in = AtomType == 0;
Pty1in = AtomType == 1;
Pty2in = AtomType == 2;%added to script

nAtoms0 = sum(Pty0in);
nAtoms1 = sum(Pty1in);
nAtoms2 = sum(Pty2in); %added to script
...
if nAtoms2
    V2_2 = (Vx(Pty2in).*Vx(Pty2in) + Vy(Pty2in).*Vy(Pty2in));%added to script
else
    V2_2 = 0;
end
...
KE2 = mean(V2_2) * Mass2 * 0.5;%added to script

KETot(c) = (KE0 * nAtoms0 + KE1 * nAtoms1 + KE2 * nAtoms2);%modified line for KE2
T(c) = KETot(c) / nAtoms / C.kb;
T0(c) = KE0 / C.kb;
T1(c) = KE1 / C.kb;
T2(c) = KE2 / C.kb; %added to script
...
while t < TStop
    ...
    elseif Method == 'VE'
        ...
        x(Pty2in) = -xpp(Pty2in) + 2 * xp(Pty2in) + dt^2 / Mass2 * Fx(Pty2in);%added to script

        y(Pty0in) = -ypp(Pty0in) + 2 * yp(Pty0in) + dt^2 / Mass0 * Fy(Pty0in);
        y(Pty1in) = -ypp(Pty1in) + 2 * yp(Pty1in) + dt^2 / Mass1 * Fy(Pty1in);
        y(Pty2in) = -ypp(Pty2in) + 2 * yp(Pty2in) + dt^2 / Mass2 * Fy(Pty2in);%added to script
    ...
    KETot(c) = (KE0 * nAtoms0 + KE1 * nAtoms1 + KE2 * nAtoms2);%modified for KE2
    T(c) = KETot(c) / nAtoms / C.kb;
    T0(c) = KE0 / C.kb;
    T1(c) = KE1 / C.kb;
