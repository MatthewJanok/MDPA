...
global Phi nAtoms time Mass0 Mass1 Mass2 Pty0in Pty1in Pty2in %Modified to add Pty2in
global LJEpsilon Phi0 PhiTot KETot MinX MaxX MinY MaxY
global T T0 T1 T2 ScaleV MarkerSize doPlotImage PlotCount %Modified to add T2
...
subplot(3, 2, 1), plot(x(Pty2in), y(Pty2in), 'ro', 'markers',...
    MarkerSize,'MarkerFaceColor', 'r');%added to script
...
AE(Pty2in) = 1 / 2 * Mass2 * V2(Pty2in) + Phi(Pty2in) - Phi0; %added to script
...
subplot(3, 2, 6), plot(time, T2, 'r', 'linewidth', 2);%added to script
%
xlabel('time')
ylabel('Temperature')
